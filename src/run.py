...
lr_score, lr_head = LogReg(x_train, y_train, x_test, y_test)
svm_score, svm_head = SupportVectorMachine(x_train, y_train, x_test, y_test)

# ~~~~~~~~~~~~~~~~~~~~

# Add scores to the array
scores = (dc_score, knn_score, rfc_score, nb_score, lr_score, svm_score)

# Name of the Algorithm
algorithms = ("Decision Tree", "K Nearest Neighbor", "Random Forest", "Naive Bayes", "Logistic Regression", "Support Vector Machine")
...
plt.xticks(h_pos, algorithms, fontsize=10)
plt.ylabel("Percent of prediction", fontsize=20)
plt.yticks(np.arange(0.00, 1.01, step=0.10))
plt.ylim(0.00, 1.00)
...
plt.bar(h_pos, scores, color=colors)
plt.grid()
plt.suptitle("Models Comparison Zoomed", fontsize=24)
plt.show()